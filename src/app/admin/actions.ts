...
export async function inviteClient(form: FormData) {
  ...
  const { data, error }=await admin.auth.admin.inviteUserByEmail(email,{data:{first_name:firstName},redirectTo:`${getTrustedSiteUrl()}/auth/callback?next=/desk/login/update-password`}); if(error) throw new Error(error.message);
...
export async function saveKnowledge(form: FormData) {
  const context=await requireAdmin();const supabase=await createClient();const organizationId=text(form,"organization_id");const payload={organization_id:organizationId,title:text(form,"title"),category:text(form,"category"),content:text(form,"content"),tags:text(form,"tags").split(",").map(item=>item.trim()).filter(Boolean),source:optional(form,"source"),created_by:context.user.id};const{data,error}=await supabase.from("knowledge_items").insert(payload).select("id").single();if(error)throw new Error(error.message);await activity(organizationId,`Knowledge added: ${payload.title}`,"knowledge",data.id);revalidatePath(`/admin/clients/${organizationId}`);revalidatePath("/desk/app/knowledge");
}
