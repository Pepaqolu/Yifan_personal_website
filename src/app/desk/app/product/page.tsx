...
export default async function ProductPage({searchParams}:{searchParams:Promise<{id?:string;website?:string}>}){
  ...
  const [intelligence,trace]=selected?await Promise.all([getProductIntelligence(context.organization!.id,selected.id),getSearchTrace(context.organization!.id,selected.id)]):[null,[]];
  ...
  return <>
    ...
    <ProductProfileWizard initial={selected} initialUrl={(params.website||"").slice(0,500)}/>
