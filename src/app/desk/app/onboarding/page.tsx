...
export default async function OnboardingPage({searchParams}:{searchParams:Promise<{website?:string|string[]}>}) {
  await requireWorkspace();const balance=await getTokenBalance();
  ...
  return <main className="mx-auto max-w-6xl py-8 sm:py-16">
    ...
    <section className="mt-16 sm:mt-24"><ProductProfileWizard initial={null} initialUrl={website} tokenBalance={balance?.available_tokens??0}/></section>
  </main>;
}
