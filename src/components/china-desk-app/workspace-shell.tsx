...
export function WorkspaceShell({ context, navigation, label, children }: { context: WorkspaceContext; navigation: readonly (readonly [string, string, string?])[]; label: string; children: ReactNode }) {
  ...
  return (
    <div className="workspace-shell min-h-screen bg-paper text-ink">
      ...
      <main className="min-h-screen bg-[radial-gradient(circle_at_60%_0%,rgba(158,215,255,0.025),transparent_26%)] px-5 py-8 sm:px-8 sm:py-12 lg:ml-72 lg:px-12 xl:px-16"><div className="mx-auto max-w-[92rem]">{trialEnds ? <div className={`mb-8 flex flex-wrap items-center justify-between gap-4 rounded-xl border px-4 py-3 text-sm ${trialExpired ? "border-accent/30 bg-accent/[0.08]" : "border-line bg-elevated"}`}><div><span className="font-medium">{trialExpired ? `Your ${productConfig.shortName} trial has ended.` : `${trialDays} day${trialDays===1?"":"s"} remaining in your ${productConfig.shortName} trial.`}</span>{trialExpired?<span className="ml-2 text-charcoal">Your opportunity map is saved.</span>:null}</div>{trialExpired?<a href={`mailto:${productConfig.email}?subject=${encodeURIComponent(`${productConfig.shortName} access`)}`} className="text-accent">Unlock continuous intelligence →</a>:<span className="font-mono text-[0.58rem] uppercase tracking-[0.1em] text-stone">No payment details required</span>}</div>:null}{children}</div></main>
    </div>
  );
}
