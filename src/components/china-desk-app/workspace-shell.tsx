  ...
  return (
    <div className="workspace-shell min-h-screen bg-paper text-ink">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-line bg-dark-soft px-6 py-7 lg:flex lg:flex-col">
        <div className="border-b border-line pb-6"><Link href="/desk" className="flex items-center gap-3 text-[0.72rem] font-semibold tracking-[0.08em]"><span className="h-2 w-2 rounded-full bg-accent shadow-[0_0_14px_rgba(158,215,255,0.28)]"/>CHINA DESK</Link><p className="mt-3 font-mono text-[0.56rem] uppercase tracking-[0.12em] text-stone">{label}</p></div>
      ...
      <header className="sticky top-0 z-20 border-b border-line bg-paper/[0.78] px-5 py-4 backdrop-blur-2xl backdrop-saturate-150 lg:hidden">
        <div className="flex items-center justify-between"><Link href="/desk" className="flex items-center gap-2 text-[0.68rem] font-semibold tracking-[0.08em]"><span className="h-1.5 w-1.5 rounded-full bg-accent"/>CHINA DESK</Link><details className="group relative"><summary className="list-none rounded-lg border border-line px-3 py-1.5 font-mono text-[0.58rem] uppercase tracking-[0.1em] text-charcoal">Menu</summary><div className="absolute right-0 top-11 w-[min(19rem,calc(100vw-2.5rem))] rounded-2xl border border-line bg-elevated p-4 shadow-[var(--shadow-elevated)]"><WorkspaceNav items={navigation}/><div className="mt-5 border-t border-line pt-4"><p className="text-xs text-stone">{displayName}</p><form action={signOut}><button className="mt-3 text-xs text-charcoal">Sign out →</button></form></div></div></details></div>
      </header>
