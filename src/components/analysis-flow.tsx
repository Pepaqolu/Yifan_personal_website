...
export function AnalysisFlow({ initialWebsite = "" }: { initialWebsite?: string }) {
  ...
  return <form action={action} onChange={begin} className="mx-auto max-w-6xl py-10 sm:py-16">
    ...
    <header className="grid gap-8 border-b border-line pb-8 sm:grid-cols-12 sm:items-end">
      ...
      <div className="sm:col-span-3 sm:col-start-10"><div className="font-mono text-[0.62rem] uppercase tracking-[0.12em] text-stone">Step {step} of 5</div></div>
    </header>
    ...
    <div className="min-h-[34rem] py-12 sm:py-16">
      ...
      {step === 3 ? <section className="grid gap-10 lg:grid-cols-12"><div className="lg:col-span-5"><p className="eyebrow text-stone">03 · INDUSTRY</p><h2 className="mt-6 text-[clamp(3rem,6vw,6.2rem)] font-medium leading-[0.9] tracking-[-0.068em]">What industry are you in?</h2><p className="mt-6 max-w-md leading-7 text-charcoal">Meridian uses this to select the right terminology, sources and specialist overlay.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:col-span-6 lg:col-start-7">{industries.map((item)=><Choice key={item} value={item} type="radio" selected={industry===item} onToggle={()=>setIndustry(item)}/>)}</div></section> : null}
    ...
    {pending ? <div role="status" aria-live="polite" className="mb-8 rounded-2xl border border-accent/25 bg-accent/[0.08] p-5"><div className="flex items-center gap-3"><span className="h-2.5 w-2.5 animate-pulse rounded-full bg-accent"/><p className="font-medium">Understanding your product and reading the supplied source…</p></div><p className="mt-2 text-sm text-charcoal">Next: interpret the evidence, score the opportunity and build your map. Meridian will not invent missing facts.</p></div> : null}
