...
export async function customerPortal(customerId:string,subscriptionId?:string|null){return request<{urls:{general:{overview:string};subscriptions?:Array<{update_subscription_payment_method?:string}>}}>(`/customers/${customerId}/portal-sessions`,"POST",subscriptionId?{subscription_ids:[subscriptionId]}:undefined);}
export async function chargeSubscription(subscriptionId:string,priceId:string){return request<{id:string}>(`/subscriptions/${subscriptionId}/charge`,"POST",{items:[{price_id:priceId,quantity:1}],effective_from:"immediately",on_payment_failure:"prevent_change"});}
